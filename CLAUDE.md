...
Before restoring/building, a licensed NuGet source for `DevExpress.Blazor` (a commercial package) must be added via `dotnet nuget add source` or a local `NuGet.Config` — never commit that source or its credentials.

## Architecture

- **DI/service registration** (`Program.cs`): the Spotify client library is wired up via `.AddSpotify(new() { AuthServerApiBase, ClientId, PermissionScopes, ... })`, plus `.AddBlazoredModal()` and `.AddDevExpressBlazor()`. `host.Services.InitializeSpotify()` runs before `host.RunAsync()`.
...
- **Styles**: no Blazor CSS isolation. Each `Pages/X` or `Shared/X` folder that has styles keeps them in its own `Styles/*.scss`, and every one of those files is explicitly `@import`-ed into the top-level `Styles/Site.scss`. `compilerconfig.json` (BuildWebCompiler) compiles that single entry point to `wwwroot/css/site.g.css` — adding a new styled component means adding its `.scss` to `Site.scss`, not just dropping the file in a folder.
